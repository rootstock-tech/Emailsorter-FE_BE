...
- POST /api/triage    -> process the whole inbox (chunks of 200) in the
                         background for the current user, returning immediately
...
from app.gmail_client import authenticate_from_token_json
...
def _default_progress():
    return {"status": "idle", "counts": None, "error": None}
...
@app.post("/api/triage")
def api_triage(request: Request, background_tasks: BackgroundTasks):
    """Start a full chained triage in the background for the current user.

    Processes the whole inbox in chunks of 200 (via triage_until_empty) until
    caught up. Returns 401 if not authenticated, 409 if a run is already in
    progress. Poll /api/triage/status for progress and the final counts.
    """
    ...
    progress.update(status="running", counts=None, error=None)
    background_tasks.add_task(_run_triage, email, service)
    return JSONResponse({"status": "started"})


def _run_triage(email, service):
    ...
    try:
        ...
        progress.update(status="done", counts=counts, error=None)
...
def _run_scheduled_triage(email):
    ...
    the same chained triage as a manual run.
    """
...
async def api_triage_schedule(request: Request):
    ...
    try:
        ...
        body = {}
    run_at_raw = body.get("run_at") if isinstance(body, dict) else None

    run_at = _parse_future_datetime(run_at_raw)
    if run_at is None:
        return JSONResponse(
